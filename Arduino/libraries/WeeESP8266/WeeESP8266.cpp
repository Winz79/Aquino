...
ESP8266::ESP8266(HardwareSerial &uart, int resetPin, uint32_t baud): m_puart(&uart)
{
	m_puart->begin(baud);
	rx_empty();
}
...
String ESP8266::recvString(String target1, String target2, String target3, String target4, uint32_t timeout)
{
	String data;
	char a;
	unsigned long start = millis();
...
bool ESP8266::sATCIPSTARTSingle(String type, String addr, uint32_t port)
{
    ...
    data = recvString("OK", "ERROR", "ALREAY CONNECT", 10000);
...
bool ESP8266::sATCIPSTARTMultiple(uint8_t mux_id, String type, String addr, uint32_t port)
{
    ...
    data = recvString("OK", "ERROR", "ALREAY CONNECT","busy", 10000);
    if (data.indexOf("OK") != -1 || data.indexOf("ALREAY CONNECT") != -1) {
        return true;
    }
    return false;
